...
class DetailOrderController extends GetxController
  ...
  Future<dynamic> getListOrderNonDki(idPo) async {
    change(null, status: RxStatus.loading());
    initGetData();
    token = CancelToken();
    List<dynamic> result = [];
        ...
        if (response['data']['list'].length == 0) {
          ...
          for (var i = 0; i < response['data']['list'].length; i++) {
            ...
            response['data']['list'][i].forEach((key, value) {
              ...
              for(var j = 0; j < response['data']['list_barang'].length; j++){
                newData['stok_barang'] = response['data']['list_barang'][j]['stok'];
              }
              combine = {...combine, ...newData};
            });
          ...
          for (var i = 0; i < result.length; i++) {
            // debugPrint('statement ${result[i]['id']}');
            textEditingController = TextEditingController(text: '');
            textEditingControllers.add(textEditingController);

            textEditingControllerVal =
                TextEditingController(text: result[i]['id'].toString());
            textEditingControllersVal.add(textEditingControllerVal);
            qtyProduk.add(result[i]['qty']);
          }
          ...
          for (var i = 0; i < response['data']['total'].length; i++) {
            debugPrint('statement ${response['data']['total'][i]['total_tagihan']}');
            if (response['data']['total'][i]['total_tagihan'] == 0) {
                setTotalProduk(0);
              } else {
                setTotalProduk(
                    response['data']['total'][i]['total_tagihan']);
              }
